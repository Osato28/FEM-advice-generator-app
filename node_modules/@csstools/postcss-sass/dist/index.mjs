  ...
  return {
    ...
    Once(root, {
      ...
      Object.assign({}, opts, requiredSassConfig, {
        ...
        const parent = resolve(postConfig.from); // use stats.includedFiles to get the full list of dependencies.  Importer will not receive relative imports.  See https://github.com/sass/dart-sass/issues/574

        for (const includedFile of stats.includedFiles) {
          // strip the #sass suffix we added
          const file = resolve(includedFile.replace(/#sass$/, '')); // don't include the parent as a dependency of itself
